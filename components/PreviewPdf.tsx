        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-2xl shadow-2xl shadow-black/20 border border-gray-200 overflow-hidden rotate-[-0.5deg] hover:rotate-0 transition-transform duration-500">
            <div className="bg-gray-50 border-b border-gray-200 px-6 py-3 flex items-center justify-between">
              ...
              <span className="text-xs text-gray-400">{t('preview.pdf_info')}</span>
            </div>
              ...
              <div className="grid grid-cols-3 md:grid-cols-6 gap-3 mb-8">
                {[
                  ...
                  <div key={s.label} className="bg-green-50 border border-green-100 rounded-xl p-3">
                    ...
                    <div className={`text-sm font-bold ${s.color || 'text-gray-900'}`}>{s.value}</div>
                  </div>
                ))}
              </div>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={i} className={i % 2 === 0 ? 'bg-gray-50' : ''}>
                      ...
                      <td className="px-3 py-2 text-center text-gray-700">{row.rides}</td>
                      <td className={`px-3 py-2 text-right font-medium`}>{cfg.symbol}{row.amount.toFixed(2)}</td>
                      <td className="px-3 py-2 text-right text-gray-700">{cfg.symbol}{row.fuel.toFixed(2)}</td>
                      <td className="px-3 py-2 text-right text-gray-700">{row.km} km</td>
