  ...
  if (rides.length === 0) {
    return (
      <div className="text-center py-12 text-gray-400">
        ...
        <div className="font-medium">Nenhuma corrida registrada</div>
        <div className="text-sm">Adicione corridas na calculadora para acompanhar seus resultados</div>
      </div>
    )
  }
    ...
    <div className="space-y-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-gray-400">
                {new Date(ride.date).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}
              </span>
            </div>
            <div className="flex gap-3 text-xs text-gray-400">
              <span>{ride.kmDriven} km</span>
              <span>Comb: {formatCurrency(ride.fuelCost)}</span>
              <span>KM: {formatCurrency(ride.costPerKm)}</span>
            </div>
          </div>
      ...
      {rides.length > 50 && (
        <div className="text-center text-sm text-gray-400 py-2">
          Mostrando as 50 corridas mais recentes de {rides.length} no total
        </div>
      )}
    </div>
  )
