...
export default function ShareButton({ result, locale }: Props) {
  ...
  const shareText = t(textKey, {
    ...
    profit: profitStr,
    score: result.score,
    km: (result.totalCost && result.costPerKm ? (result.totalCost / result.costPerKm) : 0).toFixed(1),
    fuel: formatCurrency(result.fuelCost, locale),
  })
  ...
  return (
    <div className="flex gap-2 mt-3">
      <button
        ...
        className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-[#25D366] text-white text-sm font-semibold hover:bg-[#1DA851] transition-all"
      >
