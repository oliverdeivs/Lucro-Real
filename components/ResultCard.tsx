  ...
  return (
    <div className={`rounded-2xl border-2 p-6 ${cfg.bg} ${cfg.border} animate-fadeIn`}>
      <div className="flex items-center justify-between mb-4">
        <span className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('result.titulo')}</span>
      ...
      <div className="space-y-2 mb-4">
        <div className="bg-white/60 dark:bg-gray-800/60 rounded-xl p-3 flex items-center justify-between">
          <span className="text-xs text-gray-400 dark:text-gray-500">{t('result.valor_bruto')}</span>
          <span className="text-sm font-bold">{formatCurrency(result.totalCost + result.profit, locale)}</span>
        </div>
        <div className="bg-white/60 dark:bg-gray-800/60 rounded-xl p-3 flex items-center justify-between">
          <span className="text-xs text-gray-400 dark:text-gray-500">{t('result.combustivel')}</span>
          <span className="text-sm font-medium text-warning">{formatCurrency(result.fuelCost, locale)}</span>
        </div>
        {result.otherCosts > 0 && (
          <div className="bg-white/60 dark:bg-gray-800/60 rounded-xl p-3 flex items-center justify-between">
            <span className="text-xs text-gray-400 dark:text-gray-500">{t('result.outros')}</span>
            <span className="text-sm font-medium text-orange-500">{formatCurrency(result.otherCosts, locale)}</span>
          </div>
        )}
        <div className="bg-white/60 dark:bg-gray-800/60 rounded-xl p-3 flex items-center justify-between border-t border-gray-100 dark:border-gray-800">
          <span className="text-xs font-medium text-gray-500 dark:text-gray-400">{t('result.custo_total')}</span>
          <span className="text-sm font-bold text-gray-900 dark:text-white">{formatCurrency(result.totalCost, locale)}</span>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="bg-white/60 dark:bg-gray-800/60 rounded-xl p-3">
          <span className="text-xs text-gray-400 dark:text-gray-500 block">{t('result.margem')}</span>
          <span className={`text-sm font-bold ${isPositive ? 'text-profit' : 'text-loss'}`}>
            {result.percentage.toFixed(1)}%
          </span>
        </div>
        <div className="bg-white/60 dark:bg-gray-800/60 rounded-xl p-3">
          <span className="text-xs text-gray-400 dark:text-gray-500 block">{t('result.custo_km')}</span>
          <span className="text-sm font-bold">{formatCurrency(result.costPerKm, locale)}</span>
        </div>
      </div>

      {result.profitPerHour != null && (
        <div className="bg-profit/10 dark:bg-profit/20 border border-profit/30 rounded-xl p-3 mb-4 flex items-center justify-between">
          <span className="text-xs text-gray-500 dark:text-gray-400">{t('result.ganho_hora')}</span>
          <span className="text-sm font-bold text-profit">
            {formatCurrency(result.profitPerHour, locale)}/h
          </span>
        </div>
      )}
      ...
      {onSave && (
        <button
          onClick={onSave}
          disabled={saved}
          className="w-full py-2.5 rounded-xl border-2 border-brand-500 dark:border-brand-400 text-brand-600 dark:text-brand-400 font-semibold hover:bg-brand-50 dark:hover:bg-brand-900/50 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
        >
          {saved ? (
